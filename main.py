...
from pipeline_real import ai_first_full_eval
...
app = FastAPI(title="GanadoBravo", version="v40v-ai-first")
...
@app.get("/health")
def health():
    return {"ok": True, "service": "GanadoBravo", "version": "v40v-ai-first"}
...
def evaluate_batch_internal(files: List[UploadFile], mode: str = "levante") -> List[Dict[str, Any]]:
    ...
    for f in files or []:
        ...
        # If AI-first is enabled, try it first
        try:
            if os.getenv("EVAL_MODE","ai_first") in ("ai_first","ai") and raw:
                ai = ai_first_full_eval(raw, mode)
                if isinstance(ai.get("breed"), dict) and "confidence" in ai["breed"]:
                    ai["breed_status"] = "active"
                results.append({"result": ai, "filename": getattr(f, "filename", None)})
                continue
        except Exception:
            pass
        ...
        if not use_mock and pipe is not None:
            try:
                # Recreate image from raw if available
