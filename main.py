...
app = FastAPI(title="GanadoBravo", version="v36")
...
async def debug_static():
    def ls(p: Path):
        if p.is_dir():
            entries = [("d","/"+x.name) if x.is_dir() else ("f", x.name) for x in sorted(p.iterdir())]
            if not entries:
                return "(vacío)"
            return "<br>".join([f"{t}\t{n}" for t,n in entries])
        elif p.is_file():
            return "(archivo presente)"
        return "(no existe)"
    html = "<h2>Debug static/index.html</h2>"
    html += f"<p><b>CWD:</b> {os.getcwd()}</p>"
    html += f"<p><b>BASE_DIR:</b> {BASE_DIR}</p>"
    html += "<ul>"
    html += f"<li><code>{INDEX_HTML}</code>: " + ls(INDEX_HTML.parent) + "</li>"
    html += "</ul>"
...
@app.get("/health")
def health():
    return {"ok": True, "service": "GanadoBravo", "version": "v36"}
...
def evaluate_batch_internal(files: List[UploadFile], mode: str = "levante"):
    ...
    for f in files or []:
        ...
        d = {
            ...
            "health": [],
            "breed": {"name":"Criollo (mixto)", "confidence":0.58, "explanation":"Rasgos mixtos observados"}
        }
...
async def evaluate(file: UploadFile = File(...), mode: str = Form("levante")):
    try:
        resp = evaluate_batch_internal(files=[file], mode=mode)
        return resp
...
async def evaluate_batch(files: List[UploadFile] = File(...), mode: str = Form("levante")):
    try:
        resp = evaluate_batch_internal(files=files, mode=mode)
        return resp
    except Exception as e:
        import traceback
        return JSONResponse({"error":"evaluate_failed","detail":str(e),"trace":traceback.format_exc()}, status_code=500)
