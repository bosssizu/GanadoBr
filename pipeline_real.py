...
Reglas: si BCS ≤2.5, baja dorsal/lomo/grupo/pecho/ancho; no des "posterior" alto con BCS bajo; reduce confianza de aplomos. No inventes avances si la evidencia es pobre."""
    try:
        text=_call_azure_openai_chat_image(sys_prompt,b64,timeout_s,model) if _provider()=="azure" else _call_openai_chat_image(sys_prompt,b64,timeout_s,model)
        ...
        raise RuntimeError(f"ai_rubric_error: {e}")
...
def run_breed_prompt(img_bytes:bytes)->Dict[str,Any]:
    # 100% AI; fallback a cruza si no hay API/timeout
    ...
    sys_prompt="""Eres zootecnista especialista en razas bovinas.
Clasifica la raza o cruzamiento del bovino en la imagen. Considera rasgos de Bos indicus (giba, papada, piel suelta, orejas largas), Bos taurus europeo (sin giba, oreja pequeña/mediana, cabeza compacta), cuernos, color/pigmentación, morro, línea dorsal y proporciones.
Responde SOLO este JSON:
{
  "name": "<Cebu | Brahman | Nelore | Gyr | Holstein | Angus | Hereford | Criollo | Cruza indicus×taurus | Indeterminado>",
  "confidence": 0.0-1.0,
  "explanation": "máx 140 caracteres con 2-3 rasgos visibles que justifican"
}
Si hay mezcla evidente, usa "Cruza indicus×taurus". Si la evidencia es pobre, usa "Indeterminado" con baja confianza."""
    try:
        ...
        obj=_parse_json(text)
        name=str(obj.get("name","Indeterminado")).strip()
        conf=float(obj.get("confidence",0.5))
        expl=str(obj.get("explanation","")).strip()
        return {"name":name,"confidence":round(conf,2),"explanation":expl}
    except Exception as e:
...
def format_output(agg:Dict[str,Any], health:List[Dict[str,Any]], breed:Dict[str,Any], mode:str)->Dict[str,Any]:
    ...
    if mode == "levante": decision_level, decision_text = _decide_levante(score,bcs,risk)
    elif mode == "vaca_flaca": decision_level, decision_text = _decide_vaca_flaca(score,bcs,risk)
    elif mode == "engorde": decision_level, decision_text = _decide_engorde(score,bcs,risk)
    else: decision_level, decision_text = _decide_levante(score,bcs,risk)
    ...
    return {
        ...
        "global_conf": 0.9,
        "notes": f"Evaluación pipeline real (v39r, {mode}). " + agg.get("notes",""),
