...
def _timeout()->int: return int(os.getenv("LLM_TIMEOUT_SEC","18"))
def _retries()->int: return int(os.getenv("LLM_RETRIES","2"))
def _call_openai_chat_image(prompt:str, b64_image:str, timeout_s:int)->str:
    ...
    r=requests.post(url,headers=headers,json=body,timeout=(6,timeout_s)); r.raise_for_status(); return r.json()["choices"][0]["message"]["content"]
...
def run_baseline_rubric(img_bytes:bytes, mode:str)->Dict[str,Any]:
    import random
    base=len(img_bytes); rubric=[]; rnd=random.Random(base)
    for idx,name in enumerate(MORPHOLOGICAL_METRICS):
        sc=round(rnd.uniform(5.8,8.6),2); obs="Adecuado" if sc>=7.0 else "Correcta"
        if "posterior" in name.lower() and sc>=8.0: obs="Desarrollo posterior destacado"
        if "dorsal" in name.lower() and sc<6.5: obs="Ligera concavidad dorsal"
...
Reglas: si BCS ≤2.5, baja dorsal/lomo/grupo/pecho/ancho; no des "posterior" alto con BCS bajo; reduce confianza de aplomos. No inventes avances si la evidencia es pobre."""
    ...
    for attempt in range(retries+1):
        try:
            ...
            last_err=str(e)
            if attempt<retries: time.sleep(0.6*(2**attempt))
            else: break
...
def format_output(agg:Dict[str,Any], health:List[Dict[str,Any]], breed:Dict[str,Any], mode:str)->Dict[str,Any]:
    score=agg["global_score"]+agg.get("posterior_bonus",0.0); bcs=agg["bcs"]; risk=agg["risk"]
    if bcs<2.0 or risk>=0.6: decision_level,decision_text="NO_COMPRAR","No comprar"
    elif bcs<2.4 or risk>=0.5: decision_level,decision_text="CONSIDERAR_BAJO","Considerar bajo"
    elif score>=8.2 and 2.8<=bcs<=4.5 and risk<=0.35: decision_level,decision_text="COMPRAR","Comprar"
    elif score>=7.0 and risk<=0.55: decision_level,decision_text="CONSIDERAR_ALTO","Considerar alto"
    elif score>=5.5: decision_level,decision_text="CONSIDERAR_BAJO","Considerar bajo"
    else: decision_level,decision_text="NO_COMPRAR","No comprar"
    reasons=[]
    ...
    return {"decision_level":decision_level,"decision_text":decision_text,"global_score":round(score,2),"bcs":bcs,"risk":risk,"posterior_bonus":agg.get("posterior_bonus",0),
            "global_conf":0.9,"notes":"Evaluación pipeline real (v39l-fullui-kpis, AI). "+agg.get("notes",""),
            "qc":{"visible_ratio":0.86,"stability":"alta","auction_mode":(mode=='subasta')},
            "rubric":agg["rubric"],"reasons":reasons,"health":health,"breed":breed}
