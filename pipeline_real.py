...
def format_output(agg:Dict[str,Any], health:List[Dict[str,Any]], breed:Dict[str,Any], mode:str)->Dict[str,Any]:
    ...
    # ----- Decision logic with LEVANTE tuning -----
    if mode == "levante":
        if bcs < 1.8 or risk >= 0.65:
            decision_level, decision_text = "NO_COMPRAR", "No comprar"
        elif bcs < 2.3 or risk >= 0.50:
            decision_level, decision_text = "CONSIDERAR_BAJO", "Considerar bajo"
        elif score >= 7.8 and 2.5 <= bcs <= 4.6 and risk <= 0.35:
            decision_level, decision_text = "COMPRAR", "Comprar"
        elif (score >= 5.2 and risk <= 0.45) or (bcs >= 2.7 and risk <= 0.35):
            decision_level, decision_text = "CONSIDERAR_ALTO", "Considerar alto"
        else:
            decision_level, decision_text = "CONSIDERAR_BAJO", "Considerar bajo"
    else:
        # default stricter rules (subasta/otros)
    ...
    return {
        ...
        "global_conf": 0.9,
        "notes": "Evaluación pipeline real (v39m-levante-tuned, AI). " + agg.get("notes",""),
